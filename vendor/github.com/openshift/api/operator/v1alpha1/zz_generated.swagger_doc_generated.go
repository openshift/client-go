...
var map_EtcdBackupSpec = map[string]string{
	"nodeSelector": "nodeSelector specifies which master node(s) to run the backup job on. If no selector is specified, the default node-role.kubernetes.io/control-plane label will be used. If no nodes are matched, then no backup will run.",
...
var map_EtcdBackupStatus = map[string]string{
	...
	"nodeName":   "nodeName is used for Local backups to bind the control plane node where the snapshot will be taken.",
