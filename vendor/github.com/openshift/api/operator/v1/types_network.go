...
// +openshift:validation:FeatureGateAwareXValidation:featureGate=NoOverlayMode,rule="!has(self.transport) || self.transport != 'NoOverlay' || (has(self.routeAdvertisements) && self.routeAdvertisements == 'Enabled')",message="routeAdvertisements must be Enabled when transport is NoOverlay"
...
// +openshift:validation:FeatureGateAwareXValidation:featureGate=NoOverlayMode,rule="!has(self.transport) || (has(oldSelf.transport) && self.transport == oldSelf.transport)",message="transport can only be set at installation time"
