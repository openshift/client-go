...
// +openshift:validation:FeatureGateAwareXValidation:featureGate=NoOverlayMode,rule="has(self.defaultNetworkNoOverlayOptions) == (has(self.defaultNetworkTransport) && self.defaultNetworkTransport == 'NoOverlay')",message="defaultNetworkNoOverlayOptions must be set if and only if defaultNetworkTransport is NoOverlay"
