...
var map_MachineConfigNodeSpecConfigImage = map[string]string{
	...
	"desiredImage": "desiredImage is the fully-qualified pullspec of the image that the Machine Config Operator (MCO) intends to apply to the node.",
}
...
var map_MachineConfigNodeStatus = map[string]string{
	...
	"conditions":         "conditions represent the observations of a machine config node's current state. Valid types are: UpdatePrepared, UpdateExecuted, UpdatePostActionComplete, UpdateComplete, Updated, Resumed, Drained, AppliedFilesAndOS, AppliedOSImage, AppliedFiles, Cordoned, Uncordoned, RebootedNode, NodeDegraded, PinnedImageSetsProgressing, ImagePulledFromRegistry, and PinnedImageSetsDegraded. The following types are only available when the ImageModeStatusReporting feature gate is enabled: ImagePulledFromRegistry, AppliedOSImage, AppliedFiles",
...
var map_MachineConfigNodeStatusConfigImage = map[string]string{
	...
	"currentImage": "currentImage is the fully-qualified pullspec of the image that is currently applied to the node. It is an optional field because at the beginning of an update, the node may not have an image pulled or applied, and the current image would still be the previous one while the desired image is being set. This field is updated as the node applies the new OS image.",
	"desiredImage": "desiredImage is a mirror of the desired image from the Spec. When the current and desired image are not equal, the node is in an updating phase.",
}
